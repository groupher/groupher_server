defmodule GroupherServer.Test.CMS.Comments.DocCommentReplies do
  ...
  describe "[basic article comment replies]" do
    test "exsit comment can be reply", ~m(doc user user2)a do
    ...
    test "deleted comment can not be reply", ~m(doc user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:doc, doc.id, mock_comment(), user)
    ...
    test "multi reply should belong to one parent comment", ~m(doc user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:doc, doc.id, mock_comment(), user)
    ...
    test "comment replies only contains @max_parent_replies_count replies", ~m(doc user)a do
    ...
    test "replies count should inc by 1 after got replyed", ~m(doc user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:doc, doc.id, mock_comment(), user)
  ...
  describe "[paged article comment replies]" do
    test "can get paged replies of a parent comment", ~m(doc user)a do
      {:ok, parent_comment} = CMS.create_comment(:doc, doc.id, mock_comment(), user)
    ...
    test "can get reply_to info of a parent comment", ~m(doc user)a do
      page_number = 1
      page_size = 10
